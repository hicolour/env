...
scratchpads = [
  ...
  NS "alsamixer" "roxterm --role alsamixer -e alsamixer"
    (role =? "alsamixer")
    (customFloating $ W.RationalRect 0.15 0.2 0.7 0.7),

  NS "weather" "roxterm --role weather -e curl wttr.in; bash"
    (role =? "weather")
    (customFloating $ W.RationalRect 0.15 0.2 0.7 0.7)
...
-- Commands --------------------------------------------------------------------
xmonadCleanupRecompileRestartXmonadCommand = "rm ~.xmonad/xmonad.state && killall dzen2 && xmonad --recompile && xmonad --restart"
systemRebootCommand                        = "~/scripts/close_all.sh && sleep 3 && systemctl reboot"
systemPoweroffCommand                      = "~/scripts/close_all.sh && sleep 3 && systemctl reboot"
...
-- Xmonad  ------------------------------------------------------------------------------------
  ...
  , ((modm .|. controlMask, xK_q     ), spawn xmonadCleanupRecompileRestartXmonadCommand )   -- Cleanup, recompile and restart xmonad

-- System  ------------------------------------------------------------------------------------
  , ((modm .|. controlMask, xK_r     ), spawn systemRebootCommand)                           -- Reboot
  , ((modm .|. controlMask, xK_h     ), spawn systemPoweroffCommand)                         -- Poweroff
  , ((modm,                 xK_l     ), spawn "slimlock")                                    -- Logout
-- Screen  ------------------------------------------------------------------------------------
...
-- Focus  -------------------------------------------------------------------------------------
  ...
  , ((modm,                 xK_m     ), withOtherOf2 W.shift)                                -- Move current window to other screen
  , ((modm,                 xK_u     ), onOtherOf2 W.focusUp)                                -- Focus up on other screen

-- Layout Window Navigation  ------------------------------------------------------------------
  , ((modm,                 xK_Right ), sendMessage $ Go R)                                  -- Layout navigation go right
  , ((modm,                 xK_Left  ), sendMessage $ Go L)                                  -- Layout navigation go left
  , ((modm,                 xK_Up    ), sendMessage $ Go U)                                  -- Layout navigation go up
  , ((modm,                 xK_Down  ), sendMessage $ Go D)                                  -- Layout navigation go down

-- Layout Window Position  --------------------------------------------------------------------
  , ((modm .|. controlMask, xK_Right ), sendMessage $ Swap R)                                -- Layout window position move right
  , ((modm .|. controlMask, xK_Left  ), sendMessage $ Swap L)                                -- Layout window position move left
  , ((modm .|. controlMask, xK_Up    ), sendMessage $ Swap U)                                -- Layout window position move up
  , ((modm .|. controlMask, xK_Down  ), sendMessage $ Swap D)                                -- Layout window position move down

-- Window Resizing  ---------------------------------------------------------------------------
  , ((modm .|. shiftMask,   xK_Left  ), sendMessage Shrink)                                  -- Window resizing shrink/expand left
  , ((modm .|. shiftMask,   xK_Right ), sendMessage Expand)                                  -- Window resizing shrink/expand right
  , ((modm .|. shiftMask,   xK_Up    ), sendMessage MirrorExpand)                            -- Window resizing shrink/expand up
  , ((modm .|. shiftMask,   xK_Down  ), sendMessage MirrorShrink)                            -- Window resizing shrink/expand down
...
-- Basic launchers --------------------------------------------------------------------------------
  ...
  , ((modm,                 xK_x     ), spawn "rofi -show drun -matching fuzzy")             -- Launch an app launcher
  , ((modm,                 xK_c     ), spawn "google-chrome-stable")
  , ((modm,                 xK_b     ), spawn "subl3")
  ...
  -- Cleanup, recompile and restart xmonad
  , ((modm .|. controlMask, xK_q     ), spawn "rm ~.xmonad/xmonad.state && killall dzen2 && xmonad --recompile && xmonad --restart")
  --------------------------------------------------------------------------------------------------------
  -- System 
  --------------------------------------------------------------------------------------------------------
  -- Reboot
  ...
  , ((mod1Mask,   xK_r  ), spawn "xdg-open http://www.google.com/reader/view/")
   ...
   , ((modm, xK_F9), spawn "scrot 'screenshoots/screen_%Y-%m-%d-%H-%M-%S.png' -d 1")

   --take a screenshot of focused window 
   , ((modm, xK_F10), spawn "scrot 'screenshoots/window_%Y-%m-%d-%H-%M-%S.png' -d 1 -u")
  ...
  , ((modm, xK_F9 ), namedScratchpadAction scratchpads "weather")
